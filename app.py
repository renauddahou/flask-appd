...
app.config['MYSQL_DATABASE_HOST'] = 'database-1.cnrw73i3wllh.ap-southeast-1.rds.amazonaws.com'
app.config['MYSQL_DATABASE_USER'] = 'admin'
app.config['MYSQL_DATABASE_PASSWORD'] = 'testpw0021'
app.config['MYSQL_DATABASE_DB'] = 'sawajobdb2'
...
def process2 (id_candidat,id_ofcand):
    ...
    try:
        ...
        for i in range(0,CV_cand.shape[0]):
            ...
            CV=translator(lines)
            CV=re.sub(pattern='[^a-zA-Z]', repl=' ', string=CV)
            document = Document()
        ...
        for i in range(0,descript.shape[0]):
            ...
            job=re.sub(pattern='[^a-zA-Z]', repl=' ', string=job)
        ...
        lisdoc=[str(id_candidat)+('.docx'),str(id_candidat)+('.pdf'),str(id_candidat)+('.doc')]
        ...
        for i in list(Ranked_resumes['Noms']):
            if i in lisdoc:
                ...
                Ranked_candresum="le candidat portant l'id"+ ' '+ str(id_candidat)+ ' '+ "n'a pas postuler à cette offre"
        ...
        for word in list(words): 
            if word in stopwords:
                words.remove(word)
                wordsf=[]
                [wordsf.append(x) for x in words if x not in wordsf]
        ...
        Top_skill= "You have"+' '+str(sc_sf['count'][0])+' '+str(sc_sf.skills_e[0].split())+' '+"out of"+' '+str(len(wordsf))+" top skills among all other applicants."+' '+str(wordsf)
...
###############################################################################""
# chargement de list de langues
mainlist=['Arabe', 'Tchécoslovaque', 'Danois', 'Allemand', 'Grecque', 'Anglais', 'Espagnol', 'Estonien', 'Farsi', 'Finnois', 'Français', 'Hébreu', 'Hongrois', 'Indonésien', 'Origine indéterminée', 'Italien', 'Japonnais', 'Coréen', 'Flamand', 'Norvégien', 'Portuguais', 'Roumain', 'Russe', 'Slovène', 'Albanais', 'Suédois', 'Thaï', 'Turc', 'Ukrainien', 'Yiddish', 'Chinois', 'ZoulouArab', 'Czechoslovakian', 'Danish', 'German', 'Greek', 'English', 'Spanish', 'Estonian', 'Farsi', 'Finnish', 'French', 'Hebrew', 'Hungarian', 'Indonesian', 'Indeterminate origin', 'Italian', 'Japanese', 'Korean', 'Flemish', 'Norwegian', 'Portuguese', 'Romanian', 'Russian', 'Slovenian', 'Albanian', 'Swedish', 'Thai', 'Turkish', 'Ukrainian', 'Yiddish', 'Chinese', 'Zulu']
#url="https://github.com/renauddahou/CV_exemple/blob/main/Bijjula%20Sahithi.docx?raw=true"
def cvcheck(url):
    try:
        file_name  = url.split("/")[-1]
        download(url)
        URL = 'https://jobs.lever.co/parseResume'
        resume_file= "./CVdir/"+file_name
        ...
        parsed_resume = json.dumps(response.json())
        resume.close()
        os.remove("./CVdir/"+file_name)

        data_df = pd.read_json(parsed_resume, lines=True)
        ##########################
        Positions={'Experience': data_df['positions'][0]}
        
        ################################
        Schools={'Degree': data_df['schools'][0]}
        ##################
        Skills={'skills': data_df['summary'][0].get('skills')}
        #recuopération de la liste des skills
        for i in list(Skills.values()):
            k=i.split(',')

        # recupération de la langue parlé par le candidat
        k = [x.strip(' ') for x in k]

        ls=list(set([x.lower() for x in k]) & set(list(map(str.lower,mainlist))))

        keys = ['langue_speak']
        values = [ls]
        langue_speak =dict(zip(keys, values))

        # en cas d'échec

    except Exception as e:
        err="reviewed the structure of your CV for more information\n\n"
        
    try:
        text1=extract_text_from_docx_url(url, user_agent=None)
    except:
        text2=extract_text_from_pdf_url(url, user_agent=None)
    ...
    pars=skills_doc2(text)
    for i in pars:
        Skills={'skills': i.get('skills')}
    ...
    try:
        resultat=[Skills,langue_speak,Schools,Positions]
    except:
        resultat=[err,Skills,langue_speak]
...
def predict_cand():
    id_candidat = int(request.args.get('id1'))
    id_ofcand = int(request.args.get('id2'))
...
def predict_cv():
    url = request.args.get('url').replace(" ", "%20") #mis pour les url bizar
    
    return jsonify(cvcheck(url))
